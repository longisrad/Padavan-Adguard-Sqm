...
# Detect PPPoE overhead tu dong
case "$WAN_INTF" in
    ppp*) OVERHEAD=8 ;;
    *)    OVERHEAD=0 ;;
esac

# Shape tai 95% line rate - tranh queue day o upstream ISP
UP_SHAPED=$(( $UP * 95 / 100 ))
...
setup_shaping() {
    ...
    # Tier 2 - Normal: web, stream (default)
...
# Upload shaping - egress tren WAN
setup_shaping $WAN_INTF $UP_SHAPED $UP_RT $UP_BE $UP_BK

# Download shaping - ingress qua IFB
